...
class Edit extends \Heaven7\Hungarianzipcodes\Controller\Adminhtml\Zip
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            ...
            $model->setZipCodeOrig($model->getZipCode());
        }
        $this->_coreRegistry->register('heaven7_hungarianzipcodes_zip', $model);
        
        // 5. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->resultPageFactory->create();
        $this->initPage($resultPage)->addBreadcrumb(
            $id ? __('Edit Zip') : __('New Zip'),
            $id ? __('Edit Zip') : __('New Zip')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Zip Codes'));
        $resultPage->getConfig()->getTitle()->prepend($model->getId() ? $model->getTitle() : __('New Zip'));
        return $resultPage;
    }
}
